...
final class OpenAICompatibleOptimizationService: LLMOptimizationService {
    ...
    private func userPrompt(rawText: String, cleanedText: String, dictionaryContext: String) -> String {
        let dictionarySection = dictionaryContext.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? "个人词典：无"
            : "个人词典：\n\(dictionaryContext)"
        return "\(dictionarySection)\n\n原始转写：\n\(rawText)\n\n本地清理后：\n\(cleanedText)\n\n请先理解整段话的最终意图，再输出最终要粘贴的文本。校验语义是否通顺，修正明显由语音识别造成的错别字、同音误写、术语前后不一致和语病；清理“一个、就是、然后、的话”等不影响含义的口语赘余；不要新增事实。若用户在同一段口述中表达了自我修正、否定前述、重新表述或最终意图变化，请以最终明确表达的意图为准，删除被后文明确否定或替换的前文；如果修正关系不明确，就保留原意，不要猜测。输出前请自检：是否仍有前后不一致的英文术语、明显错词或生硬语病，例如“web coding/外部 coding”应根据上下文统一为“vibe coding”，“表达自己的意识”应改为“表达自己的意思”，“你是不明确的”应改为“表达不够明确”。只输出最终要粘贴的文本，不要解释。"
    }
    ...
    static func defaultPromptTemplate(for mode: DictationMode) -> String {
        ...
        case .literal:
            ...
            - 只删除明显口头禅、重复音、无意义停顿。
            - 如果同一段口述里出现自我修正、否定前述、重新表述或最终意图变化，先理解整段最终意图；只有修正关系明确时才删除被后文替换的前文，不明确时保留原文，不要猜测。
        ...
        case .general:
            ...
            - 如果同一段口述里出现自我修正、否定前述、重新表述或最终意图变化，先理解整段最终意图；以最后明确表达的意图为准，删除被后文明确替换的前文。
            - 只有修正关系明确时才删改前文；如果不确定是在修正还是继续描述问题，保留原意，不要脑补。
        ...
        case .codingPrompt:
            ...
            5. 最终意图识别：如果同一段口述里出现自我修正、否定前述、重新表述或最终意图变化，先理解整段最终意图，再输出整理后的最终需求。

            重要原则：
            - 只输出最终要粘贴/发送的文本，不要解释你的修改过程。
            - 保留用户真实意图，不新增事实，不编造技术细节。
            - 以用户最后明确表达的意图为准；删除被后文明确否定、替换或修正的前文。
            - 只有修正关系明确时才删改前文；如果不确定是在修正还是继续描述 bug/需求，保留原意，不要脑补。
            - 校验语义是否通顺，修正明显由语音识别造成的错别字、同音误写和语病。
