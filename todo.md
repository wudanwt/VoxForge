...
### [x] T2. 音频 buffer 改用 AsyncStream 保序转发给识别引擎（7010f40）
...
### [x] T3. 剪贴板恢复前用 changeCount 防止覆盖用户数据（7010f40）
...
### [x] T4. LLM 响应清洗：剥 markdown 围栏与解释性前言（7010f40）
...
### [x] T5. readyToSubmit 状态显示 HUD 提示（去掉 NSApp.isActive 条件）（7010f40）
...
### [x] T6. 粘贴前验证目标应用已激活，失败则不发 Cmd+V（7010f40）
...
### [x] T7. WhisperKit 流式 / Apple 听写失败后重建引擎（7010f40）
...
### [x] T8. 监听音频设备配置变化，录音中断流时明确报错（7010f40）
...
### [x] T9. 清理词典→热词断链与无谓的引擎重建（7010f40）
...
### [x] T10. `autoSubmitAfterDictation` 死代码处理（7010f40）
...
### [x] T11. 中断 `.inserting` 阶段的语义修正（7010f40）
...
### [x] T14. 后处理 filler 删除加词边界，保留换行（7010f40）
...
### [x] T15. HistoryStore 读写失败不再静默（7010f40）
...
### [x] T18. 收敛 `applicationSupportDirectory.first!` force unwrap（7010f40）
...
### [x] T20. AppModel 状态机核心路径测试（7010f40）
