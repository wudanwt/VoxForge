...
词典不是普通的“查找替换”。开启大模型优化时，VoxForge 声铸会把个人词典作为上下文交给大模型，让它结合原始转写、当前文本和词条说明判断是否应改成标准写法。

未开启大模型时，只会对你明确填写的“常见误听”做本地兜底，例如把“五律 / 无虑”改成“吴律”；如果只填标准词条而没有填写常见误听，本地模式不会凭空猜测。

启用词条的标准写法会提供给支持热词的识别后端，用于增强人名、专有名词、项目名、文件名、命令和技术术语识别。常见误听不会作为热词强化给识别模型。词典配置保存在本机 `UserDefaults` 中，可在设置页恢复默认词条。
